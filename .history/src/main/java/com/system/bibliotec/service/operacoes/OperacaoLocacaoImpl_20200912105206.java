...
public class OperacaoLocacaoImpl implements IOperacaoLocacao {
	...
	@Transactional
	@Override
	public LocacaoVM despacharSolicitacaoLocacao(DespachoSolicitacaoLocacaoDTO dto) {
		...
		throw new OperacaoCanceladaException("Triagem de Documentação para Locação. "+ ex.getMessage());
		}
		...
		try {
			...
			solicitacaoService.updateStatusAndDescricao(Status.RECUSADA, dto.getIdSolicitacao(), ex.getMessage(), true);
			throw new OperacaoCanceladaException("Triagem de Validação do Exemplar escolhido. "+ ex.getMessage());
		}
