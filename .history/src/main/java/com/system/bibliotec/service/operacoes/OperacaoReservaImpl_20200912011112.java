...
import com.system.bibliotec.repository.solicitacaoService;
...
public class OperacaoReservaImpl implements IOperacaoReserva {
	...
	public ReservaVM reservaLivro(SolicitacaoReservaDTO dto) {
		...
		if(SecurityUtils.isCurrentUserInRole(AuthoritiesConstantsUltis.ROLE_ADMIN) || 
			...
			cliente = userService.findOneByUsuarioContexto();  //user anonymous online... 
			s.setUsuario(cliente);
		}
		...
		try {
			...
			solicitacaoService.updateStatusAndDescricao(s, Status.RECUSADA, "Item não permitido devido " + e.getMessage(), true);

			throw new OperacaoCanceladaException("Operação não permitida. Foi detectado inconsistência em seus dados. "+ e.getMessage());
		}
		...
		try {
			...
			s.setRejeitado(true);
			s.setDescricao("Item não permitido devido " + e.getMessage());
