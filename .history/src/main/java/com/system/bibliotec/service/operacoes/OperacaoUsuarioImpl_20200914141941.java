...
public class OperacaoUsuarioImpl implements IOperacaoUsuario {
	...
	public Usuario findOneByEmailOrUsername(String emailOrUsername) {
		...
		userRepository.findOneByEmailIgnoreCase(emailOrUsername).orElseGet(()-> userRepository.findOneByUserName(emailOrUsername).orElseThrow(
				() -> UsuarioNaoEncontrado("Usuario não localizado"))))))
		return null;
	}
