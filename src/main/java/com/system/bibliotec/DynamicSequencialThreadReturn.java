...
public class DynamicSequencialThreadReturn {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        // Exemplo de captura de retorno dinamicamente de forma sequêncial
...
//  ###########     GRUPO 2  ###########
        ...
        List<CompletableFuture<UserMockVONotDynamicReturn>> runningThreadsAll = new ArrayList<>();
        ...
        CompletableFuture<String> threadMockCheckSequencial = CompletableFuture.supplyAsync(() -> {

            boolean finishAll = false;
            while(!finishAll){
                for(int iterator = 0; iterator <= runningThreadsAll.size(); iterator++ ){

                    if(runningThreadsAll.isEmpty()) {
                        finishAll = true;
                        System.out.println();
                        System.out.println("---- Finish All ----- ");
                        break;
                    }else if(iterator >= runningThreadsAll.size()){
                        break;
                    }else if(iterator < 0){
                        iterator++;
                    }

                    CompletableFuture<UserMockVONotDynamicReturn> threadRunningInstance = runningThreadsAll.get(iterator);

                    if(threadRunningInstance.isDone()){
                        try {
                            //Poderia realizar alguma operação
                            System.out.println(threadRunningInstance.get());
                        } catch (Exception e) {
                            throw new RuntimeException(e);
                        }

                        runningThreadsAll.remove(iterator);
                        iterator = iterator - 1;
                    }
                }
            ...
            return "Finish check mock thread sequencial";
        },
        executorThreadMain);

        threadMockCheckSequencial.join();

        System.out.println(threadMockCheckSequencial.get());
...
class UserMockVO {
    ...
    public UserMockVO(String name, int age) {
        this.name = name;
        this.age = age;
    }
