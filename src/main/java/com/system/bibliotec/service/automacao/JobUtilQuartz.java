...
import com.system.bibliotec.config.quartz.PersistableCronTriggerFactoryBean;

public class JobUtilQuartz {
    ...
    public static JobDetail createJob(Class<? extends QuartzJobBean> jobClass, boolean isDurable,
                                      ApplicationContext context, String jobName, String jobGroup, String emailSolicitante,
        ...
        factoryBean.setDurability(isDurable);    // isDurable é se será persistido e mantido na base de dados do quartz mesmo apos sua conclusão..
        factoryBean.setApplicationContext(context);
        factoryBean.setName(jobName);
        factoryBean.setGroup(jobGroup);

        jobDataMap.put("emailSolicitante", emailSolicitante);
        jobDataMap.put("nomeSolicitante", nomeSolicitante);
        jobDataMap.put("tipoServicoTrabalho", tipoServicoTrabalho.toString());

        factoryBean.setJobDataMap(jobDataMap);

        factoryBean.setRequestsRecovery(true); // true em caso do sistema se desligar o job retornar o processamento ate sua conclusão e ficar com status como completed
    ...
    public static JobDetail createJob(Class<? extends QuartzJobBean> jobClass, boolean isDurable,
        ...
        factoryBean.setJobDataMap(jobDataMap);
        factoryBean.setRequestsRecovery(true); // true em caso do sistema desligar retornar o processamento do job
        factoryBean.afterPropertiesSet();
    ...
    public static Trigger createCronTrigger(String triggerName, Date startTime, String cronExpression,
        ...
        factoryBean.setTimeZone(TimeZone.getTimeZone("America/Sao_Paulo"));
